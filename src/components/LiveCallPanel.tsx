    ...
    return (
      <div className="rounded-2xl border border-border/50 bg-card p-5">
        ...
        <div className="space-y-2 text-sm">
          {data?.callerName && (
            <div className="flex items-center gap-2">
              <User className="h-3.5 w-3.5 shrink-0 text-[#0693e3]" />
          ...
          {data?.urgency && data.urgency.toLowerCase() !== "niedrig" && (
            <div className="flex items-center gap-2">
              <AlertCircle className="h-3.5 w-3.5 shrink-0 text-[#0693e3]" />
              <span className="font-medium text-[#0693e3]">Dringlichkeit: {data.urgency}</span>
            </div>
          )}
          {call.duration && (
